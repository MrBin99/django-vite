...
class DjangoViteConfig(NamedTuple):
    ...
    # If this does not begin with a slash, it will be considered an
    # asset path and looked up with the staticfiles storage.
...
class ManifestClient:
    ...
    def _clean_manifest_path(self) -> Path:
        ...
        if not initial_manifest_path:
            ...
            if not initial_manifest_path.startswith("/"):
                if not apps.is_installed("django.contrib.staticfiles"):
                    raise ImproperlyConfigured(
                        f"Manifest path for {self.app_name!r} is {initial_manifest_path!r}. "
                        "If you don't provide an absolute path to a manifest file, "
                        "you must have django.contrib.staticfiles installed to locate it.",
                    )
                from django.contrib.staticfiles.finders import find

                found_manifest_path = find(initial_manifest_path)
                if not found_manifest_path:
                    raise ImproperlyConfigured(
                        f"Manifest path for {self.app_name!r} is {initial_manifest_path!r}, "
                        "but such a path could not be found by django.contrib.staticfiles.",
                    )
                return Path(found_manifest_path)
